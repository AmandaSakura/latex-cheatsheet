\documentclass[10pt,landscape,a4paper,fleqn]{article}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage[T1]{fontenc}
%\usepackage[LY1,T1]{fontenc}
%\usepackage{frutigernext}
%\usepackage[lf,minionint]{MinionPro}
\usepackage{amsmath,amssymb}
\usepackage{tikz}
\usetikzlibrary{shapes,positioning,arrows,fit,calc,graphs,graphs.standard}
\IfFileExists{kpfonts.sty}{\usepackage[nosf]{kpfonts}}{\usepackage{lmodern}}
\IfFileExists{sourcesanspro.sty}{\usepackage[t1]{sourcesanspro}}{\usepackage[scaled=.92]{helvet}}
\usepackage{multicol}
\usepackage{wrapfig}
\usepackage[top=4mm,bottom=4mm,left=4mm,right=4mm]{geometry}
\usepackage[framemethod=tikz]{mdframed}
\usepackage{microtype}
\usepackage{pdfpages}

\let\bar\overline

\input{inhalt/def}

\begin{document}
\footnotesize
\begin{multicols}{2}
\input{inhalt/aussagen}
\input{inhalt/mengen}
\input{inhalt/relationen}
\input{inhalt/abbildungen}
%\input{inhalt/beweise} 
\input{inhalt/graphen}
%\input{inhalt/graphalgo}
%\input{inhalt/bool}
%\input{inhalt/formeln}
\end{multicols}
\end{document}
